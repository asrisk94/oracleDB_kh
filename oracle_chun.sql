...
select department_name || '의 정원은 ' || to_char(capacity) || '명입니다.' "학과별 정원"
from tb_department;
...
select student_name, student_ssn, absence_yn
from tb_student
where (substr(tb_student.student_ssn, 8, 1) = 2 or substr(tb_student.student_ssn, 8, 1) = 4)
...
from tb_student
where absence_yn like 'N' and 
      to_char(entrance_date, 'yy/mm/dd') like '02%' and
      instr(student_address, '전주시') != 0;
